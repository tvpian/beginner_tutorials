...
class MinimalPublisher : public rclcpp::Node {
 ...
 private:
  void wait_for_all_acked() {
    ...
    // Note that if no subscription is connected, wait_for_all_acked() always
    // return true.
    if (publisher_->wait_for_all_acked(wait_timeout_)) {
      RCLCPP_INFO(this->get_logger(), "All subscribers acknowledge messages");
    } else {
      RCLCPP_INFO(this->get_logger(),
                  "Not all subscribers acknowledge messages during %" PRId64
                  " ms",
  ...
  void timer_callback() {
    ...
    // After sending some messages, you can call wait_for_all_acked() to confirm
