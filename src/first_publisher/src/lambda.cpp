...
/**
 * @brief Sample class to demonstrate the use of Ros2 publisher and server node simultaneously
 * 
 */
class MyFirstPubSev : public rclcpp::Node {
 public:
    MyFirstPubSev() : Node("my_first_publisher"), count_(0) {
        auto param_desc = rcl_interfaces::msg::ParameterDescriptor();
        param_desc.description = "Set callback frequency.";
        this->declare_parameter("freq",
        2.0, param_desc);  // default =  2 hz
        ...
        publisher_ = this->create_publisher<std_msgs::msg::String>(topicname,
         10);
        timer_ = this->create_wall_timer(std::chrono
        ::milliseconds(static_cast<int> ((1000 / freq))),
        ...
        server_ = this->create_service<custom_services::srv::PassMsg>(
            "change_msg",
            std::bind(&MyFirstPubSev::server_callback, this,
 ...
 private:
    ...
    void publisher_timer_callback() {
        ...
        RCLCPP_INFO(this->get_logger(), "I published: '%s'",
         message.data.c_str());
        if (count_ >= 5) {
     ...
     * @param new_msg 
     */
    void change_string(std::string new_msg) {
        test_msg = new_msg;
    }
     ...
     * @param response 
     */ 
    void server_callback(const std::shared_ptr<custom_services::
    srv::PassMsg::Request> request,
...
int main(int argc, char **argv) {
    ...
    auto node = std::make_shared<MyFirstPubSev>();
