 ...
 public:
  MinimalContentFilteringSubscriber()
    ...
    subscription_ = this->create_subscription<std_msgs::msg::String>(
        "topic", 10,
        std::bind(&MinimalContentFilteringSubscriber::topic_callback, this, _1),
        sub_options);
    ...
    if (!subscription_->is_cft_enabled()) {
      ...
      RCLCPP_INFO(this->get_logger(),
                  "Subscribed to topic \"%s\" with content filtering",
  ...
  void topic_callback(const std_msgs::msg::String& msg) {
    ...
    if (subscription_->is_cft_enabled()) {
      ...
      current_expression_parameter_ =
          "'Hello, world! " + std::to_string(count_) + "'";
      try {
        subscription_->set_content_filter(current_filtering_expression_,
                                          {current_expression_parameter_});
    ...
    if (subscription_->is_cft_enabled()) {
      ...
      try {
        ...
        RCLCPP_INFO(
            ...
            options.filter_expression.c_str(),
            options.expression_parameters[0].c_str());
      } catch (const std::exception& e) {
        RCLCPP_ERROR(this->get_logger(), "%s", e.what());
      }
    }
