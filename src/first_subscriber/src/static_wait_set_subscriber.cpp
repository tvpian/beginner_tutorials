...
/* This example creates a subclass of Node and uses static a wait-set based loop
 * to wait on a subscription to have messages available and then handles them
 * manually without an executor */
 ...
 public:
  explicit StaticWaitSetSubscriber(rclcpp::NodeOptions options)
      : Node("static_wait_set_subscriber", options),
        ...
        wait_set_(std::array<MyStaticWaitSet::SubscriptionEntry, 1>{
            {{subscription_}}}),
  ...
  void spin_wait_set() {
    while (rclcpp::ok()) {
      // Wait for the subscriber event to trigger. Set a 1 ms margin to trigger
      // a timeout.
        ...
        case rclcpp::WaitResultKind::Timeout:
          if (rclcpp::ok()) {
            RCLCPP_WARN(this->get_logger(),
                        "Timeout. No message received after given wait-time");
          }
          break;
        default:
