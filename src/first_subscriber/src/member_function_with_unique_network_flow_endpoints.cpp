  ...
  MinimalSubscriberWithUniqueNetworkFlowEndpoints()
    ...
    try {
      ...
      subscription_1_ = this->create_subscription<std_msgs::msg::String>(
          "topic_1", 10,
          std::bind(&MinimalSubscriberWithUniqueNetworkFlowEndpoints::
                        topic_1_callback,
                    this, _1),
          options_1);
      ...
      auto network_flow_endpoints_1 =
          subscription_1_->get_network_flow_endpoints();
      auto network_flow_endpoints_2 =
          subscription_2_->get_network_flow_endpoints();
      ...
      for (auto network_flow_endpoint_1 : network_flow_endpoints_1) {
        for (auto network_flow_endpoint_2 : network_flow_endpoints_2) {
          if (network_flow_endpoint_1 == network_flow_endpoint_2) {
            RCLCPP_ERROR(
                this->get_logger(),
                "Network flow endpoints across subscriptions are not unique");
            break;
          }
        }
      }
...
int main(int argc, char* argv[]) {
  ...
  rclcpp::spin(
      std::make_shared<MinimalSubscriberWithUniqueNetworkFlowEndpoints>());
  rclcpp::shutdown();
  return 0;
}
