...
/* This example creates a subclass of Node and uses a wait-set based loop to
 * periodically poll for messages in a timer callback using a wait-set based
 * loop. */
...
class TimeTriggeredWaitSetSubscriber : public rclcpp::Node {
 public:
  ...
  ~TimeTriggeredWaitSetSubscriber() { thread_.join(); }

  void spin_wait_set() {
    while (rclcpp::ok()) {
      // Wait for the timer event to trigger. Set a 1 ms margin to trigger a
      // timeout.
